...
//  Description :   Template for implementation Sprite files
...
{ 
	...
	BitBlt(pBackBuffer->GetBackDC(), iX, iY, iW, iH, s_hSharedSpriteDC, 0, 0, SRCAND);

	SelectObject(s_hSharedSpriteDC, m_hSprite);

	BitBlt(pBackBuffer->GetBackDC(), iX, iY, iW, iH, s_hSharedSpriteDC, 0, 0, SRCPAINT);
